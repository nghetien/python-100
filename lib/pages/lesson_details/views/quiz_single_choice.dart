...
class QuizSingleChoice extends StatefulWidget {
  final QuestionData questionData;
  final int index;
  final Function(int) onChangeSelect;
  ...
  @override
  _QuizSingleChoiceState createState() => _QuizSingleChoiceState();
}

class _QuizSingleChoiceState extends State<QuizSingleChoice> {
        ...
        case $markdown:
          listItem.add(
            answerKatexToHtml(
              ...
              onSelect: () {
                setState(() {
                  myAnswer = indexAnswerChoice;
                });
                widget.onChangeSelect(indexAnswerChoice);
              },
            ),
          );
          break;
  ...
  _body() {
    if (listAnswerChoice.isNotEmpty) {
      ...
      for (int index = 0; index < listAnswerChoice.length; index++) {
        listChoice.add(_choiceItem(
          onChoiceAnswer: (value) {
            if (value == true) {
              setState(() {
                myAnswer = index;
              });
              widget.onChangeSelect(index);
            }
          },
          index: index,
        ));
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            AppLocalizations.of(context)!.select_one_answer,
